...
<div id="reviews" class="woocommerce-Reviews">
	<div id="comments">
		...
		<?php if ( have_comments() ) : ?>
			...
			if( method_exists( $product, 'get_id' ) ) {
				do_action( 'ivole_reviews_summary', $product->get_id() );
			} else {
				do_action( 'ivole_reviews_summary', $product->id );
			}
	...
	<?php if ( get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) : ?>
		...
		<div id="review_form_wrapper" class="addcommenttoproduct" style="display: none;">
			<div id="review_form">
				<?php
					...
					if( function_exists( 'wc_get_page_permalink' ) ) {
						...
						if ( $page_id = wc_get_page_id( 'myaccount' ) ) {
							...
							$comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( __( 'You must be <a href="%s">logged in</a> to post a review.', 'woocommerce' ), esc_url( $permalink ) ) . '</p>';
						}
					}
