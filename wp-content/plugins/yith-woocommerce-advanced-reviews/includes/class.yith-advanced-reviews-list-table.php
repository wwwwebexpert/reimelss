	...
	class YITH_Advanced_Reviews_List_Table extends WP_List_Table {
		...
		public function get_columns() {
			$columns = array(
				...
				YITH_YWAR_TABLE_COLUMN_REVIEW_AUTHOR  => __( 'Author', 'yith-woocommerce-advanced-reviews' ),
				YITH_YWAR_TABLE_COLUMN_REVIEW_CONTENT => __( 'Review', 'yith-woocommerce-advanced-reviews' ),
				YITH_YWAR_TABLE_COLUMN_REVIEW_DATE    => __( 'Date', 'yith-woocommerce-advanced-reviews' ),
				YITH_YWAR_TABLE_COLUMN_REVIEW_RATING  => __( 'Rate', 'yith-woocommerce-advanced-reviews' ),
				YITH_YWAR_TABLE_COLUMN_REVIEW_PRODUCT => __( 'Product', 'yith-woocommerce-advanced-reviews' ),
			);
		...
		public function column_default( $post, $column_name ) {
			...
			switch ( $column_name ) {
				...
				case YITH_YWAR_TABLE_COLUMN_REVIEW_CONTENT:
					...
					$title = empty( $post->post_title ) ? __( '[No Title]', 'yith-woocommerce-advanced-reviews' ) : wc_trim_string( $post->post_title, 80 );
				...
				case YITH_YWAR_TABLE_COLUMN_REVIEW_AUTHOR:
					...
					if ( $author_user && ! $is_custom_user ) {
						...
						$review_author_name  = ! empty( $postmeta ) ? $postmeta : __( 'Anonymous', 'yith-woocommerce-advanced-reviews' );
					...
					if ( $post->post_author ) {
						...
						if ( $user ) {
							echo '<a href="' . get_edit_user_link( $user->ID ) . '" class="review-author">' . sprintf( "%s (%s)", $user->display_name, $user->user_email ) . '</span>';

						} else {
							echo '<span class="review-author">' . __( 'Anonymous', 'yith-woocommerce-advanced-reviews' ) . '</span>';
						}

					} else {
						$author_name = yit_get_post_meta( $post->ID, YITH_YWAR_META_REVIEW_AUTHOR );

						echo $author_name ? $author_name : __( 'Anonymous', 'yith-woocommerce-advanced-reviews' );
					}*/
